...
def plot_barchart(df):
    ...
    data_chart = pd.DataFrame(plot_df.head(100))
    #fig = plt.figure()
    return st.bar_chart(data_chart)
...
if uploaded_file:
    ...
    st.markdown("### Introductory")
    st.text("Cookie Cats is a hugely popular mobile puzzle game developed by Tactile Entertainment")
    st.text("It's a classic connect three-style puzzle game where the player must connect tiles of")
    st.text("the same color to clear the board and win the level.")

    st.markdown('### Cookie Cats')
    img =  Image.open("cookiecat.png")
    st.image(img)
    ...
    # Data Exploration
    # Distribution of players for different round number
    st.write('The distribution of players in games')
    plot_barchart(df)
    
    st.markdown("AB Testing Steps")
    "Testing Steps"
    "1.creatting smaples of dataset (bootstraping)"
    ...
    "5.set signifance level and calculate the p-value to compare"
    "6.evaluate the result"

    # 1 or 7 day retention rate distribition
    st.write('1 or 7 day retention rate distribition')
